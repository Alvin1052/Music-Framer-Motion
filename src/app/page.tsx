...
type statusType = 'loading' | 'pause' | 'playing';
const Home = () => {
  const delays = [0, 0.1, 0.2, 0.3, 0.4];
  const [status, setStatus] = useState<statusType>('pause');
  ...
  const handleColor =
    status === 'playing' ? 'bg-primary-300' : 'bg-neutral-500';

  const handleOpacity = status === 'loading' ? 'opacity-50' : 'opacity-100';

  if (!status) return setStatus('loading');
  ...
  return (
    <div className=' flex items-center justify-center min-h-screen min-w-screen bg-neutral-950'>
      <div
        ...
        style={{
          boxShadow: `1px 1px 20px ${status === 'playing' ? '#8B5CF6' : ''}`,
        }}
      >
        ...
        <div className='flex flex-col'>
          ...
          <div className=' flex gap-24'>
            ...
            <motion.div
              ...
              animate={{
                rotate: status === 'playing' ? 360 : 0,
                scale:
                  status === 'playing' ? 1 : status === 'pause' ? 0.95 : 0.9,
              }}
              transition={{
                rotate: {
                  repeat: status === 'playing' ? Infinity : 0,
                  duration: status === 'playing' ? 15 : 0,
                  ease: 'linear',
                },
          ...
          <div className={`mx-144 -mt-10 bars items-end ${handleOpacity}`}>
            {delays.map((d, i) => (
              <motion.div
                ...
                transition={{
                  height: {
                    duration: status === 'playing' ? 0.5 : 0.2,
                    repeat: status === 'playing' ? Infinity : 0,
                    delay: status === 'playing' ? d : 0,
                    ease: 'easeInOut',
                  },
                  duration: 0.3,
                }}
